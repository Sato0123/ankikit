"""`ankikit eng` — 英単語 JSON を読んでカードにし、Anki まで一気に反映する。

    uv run ankikit eng words.json

やることは 4 つ。**どれかで転んでも、通るものは通す**（重複 1 件で全部止まらない）。

    1. JSON を検証して例文を空欄化   （壊れた行だけ落として理由を出す）
    2. 単語をキーに重複を除外         （デッキに既にある語 / ファイル内の重複）
    3. decks/<slug>/cards/YYYY-MM-DD.md に追記してコミット
    4. Anki へ push

`decks/` がカードの正、という原則は崩さない。ただし承認（ブランチ→main のマージ）は、
自分で打ち込んだデータには意味が無いので飛ばす。代わりに **main 上でしか実行できない**
ようにして「Anki に入っているもの = main にあるもの」を保つ。
"""
...
NAME = "eng"
HELP = "英単語 JSON をカードにして Anki まで反映"
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("file", help="英単語 JSON のパス")
    parser.add_argument("--deck", help=f"対象デッキの slug（既定 {DEFAULT_DECK}、JSON の \"deck\" が優先）")
...
def _resolve_deck(args: argparse.Namespace, loaded: vocab.Loaded) -> Deck | None:
    slug = args.deck or loaded.deck or DEFAULT_DECK
    deck = find_deck(slug)
    if deck is not None:
        return deck
    available = ", ".join(d.slug for d in load_decks()) or "(なし)"
    common.error(f"デッキ '{slug}' が見つかりません。利用可能: {available}")
    common.error(f"（作るなら `uv run ankikit new {slug} --anki-deck '英語::語彙'`）")
    return None
...
def _commit(target: Path, deck: Deck, count: int) -> bool:
    ...
    try:
        ...
        approval.git("commit", "-m", f"cards({deck.slug}): {count}枚 (ankikit eng)", "--", relative)
