...
# 「既に答えられた」カードに付くタグの接頭辞。値は理解度（KNOWN_INTERVALS のキー）。
KNOWN_TAG_PREFIX = "known"

# 理解度 → 初期間隔（日）。**Anki に新規追加するときにだけ**与える下駄で、
# 一度でも復習したカードには二度と触らない（履歴を壊さないため）。
#
# 新規カードは「明日また出る」ところから始まるが、既に答えられたものを同じ扱いにすると
# 復習の枠を食い潰す。かといって入れないと「当時は答えられたが後で忘れた」を拾えない。
# 間隔を先に進めた状態で入れることで、**忘れたものより弱く、しかし確実に**出るようにする。
KNOWN_INTERVALS: dict[int, int] = {
    1: 2,  # 答えられたが説明が詰まった
    2: 5,  # 答えられた
    3: 12,  # 即答できた
    4: 25,  # 体に入っている（念のため置いておくだけ）
}
