...
本から拾った英単語は対話に向かないので、JSON を書いて一気に流し込む。

```
uv run ankikit eng words.json     ← 検証 → 重複除外 → decks/ に追記 → コミット → Anki 反映
```
...
例文の該当語が自動で `____` になる（表面 `Let's try ____.` / 裏面 `anyway`）。
